/**
 * Tests for The Chain scoring logic.
 * TDD: These tests define the expected scoring behavior.
 */
...
describe("generateChainEmojiGrid", () => {
  it("generates DNF emoji grid for incomplete", () => {
    const score: ChainScore = {
      points: 0,
      maxPoints: 7,
      stepsTaken: 0,
    ...
    expect(generateChainEmojiGrid(score)).toBe("💀 DNF");
  });

  it("generates eagle emoji grid with negative par difference", () => {
    const score: ChainScore = {
      points: 7,
      maxPoints: 7,
      stepsTaken: 3,
    ...
    expect(generateChainEmojiGrid(score)).toBe("🦅 3 steps (-2)");
  });

  it("generates par emoji grid with E for even", () => {
    const score: ChainScore = {
      points: 5,
      maxPoints: 7,
      stepsTaken: 5,
    ...
    expect(generateChainEmojiGrid(score)).toBe("⛳ 5 steps (E)");
  });

  it("generates bogey emoji grid with positive par difference", () => {
    const score: ChainScore = {
      points: 4,
      maxPoints: 7,
      stepsTaken: 6,
    ...
    expect(generateChainEmojiGrid(score)).toBe("😐 6 steps (+1)");
  });
});
